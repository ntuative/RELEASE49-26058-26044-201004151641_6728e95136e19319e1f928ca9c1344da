package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit
   {
      ...
      private function createView() : void
      {
         ...
         _view.visible = false;
         var_316 = _view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(var_293 && var_1338)
         {
            _loc5_ = var_47.avatarRenderManager.createAvatarImage(figure,AvatarScaleType.SMALL,var_624);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_40,parseInt(FigureData.const_696));
         ...
         if(var_316)
         {
            if(false)
            {
               var_316.bitmap.dispose();
            }
            var_316.bitmap = new BitmapData(var_316.width,var_316.height,true,0);
            _loc7_ = (0 - _loc1_.width) / 2;
            _loc8_ = (0 - _loc1_.height) / 2;
            var_316.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               var_293 = var_47.figureData.getFigureString();
               var_624 = var_47.gender;
               var_47.handler.saveWardrobeOutfit(var_1571,this);
               updateView();
               break;
            case "get_button":
               if(var_293)
               {
                  var_47.loadAvatarInEditor(var_293,var_624,var_47.clubMemberLevel);
               }
         }
      }
   }
