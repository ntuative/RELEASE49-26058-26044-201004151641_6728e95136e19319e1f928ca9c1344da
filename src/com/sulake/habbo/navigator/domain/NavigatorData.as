package com.sulake.habbo.navigator.domain
{
   import com.sulake.habbo.communication.messages.incoming.navigator.FlatCategory;
   import com.sulake.habbo.communication.messages.incoming.navigator.GuestRoomData;
   import com.sulake.habbo.communication.messages.incoming.navigator.GuestRoomSearchResultData;
   import com.sulake.habbo.communication.messages.incoming.navigator.MsgWithRequestId;
   import com.sulake.habbo.communication.messages.incoming.navigator.OfficialRoomsData;
   import com.sulake.habbo.communication.messages.incoming.navigator.PopularRoomTagsData;
   import com.sulake.habbo.communication.messages.incoming.navigator.PublicRoomShortData;
   import com.sulake.habbo.communication.messages.incoming.navigator.RoomEventData;
   import com.sulake.habbo.communication.messages.parser.navigator.FavouritesMessageParser;
   import com.sulake.habbo.communication.messages.parser.navigator.NavigatorSettingsMessageParser;
   import com.sulake.habbo.communication.messages.parser.room.engine.RoomEntryInfoMessageParser;
   import com.sulake.habbo.navigator.HabboNavigator;
   import flash.utils.Dictionary;
   
   public class NavigatorData
   {
      
      private static const const_1435:int = 10;
       
      
      private var var_1577:NavigatorSettingsMessageParser;
      
      private var var_1347:int;
      
      private var var_2193:int;
      
      private var var_2194:Boolean;
      
      private var var_1092:Array;
      
      private var var_913:Dictionary;
      
      private var var_1091:Array;
      
      private var var_2339:int;
      
      private var var_2190:int;
      
      private var var_2035:int;
      
      private var var_2192:int;
      
      private var var_626:PublicRoomShortData;
      
      private var var_446:RoomEventData;
      
      private var var_134:MsgWithRequestId;
      
      private var var_2191:Boolean;
      
      private var _navigator:HabboNavigator;
      
      private var var_2195:Boolean;
      
      private var var_204:GuestRoomData;
      
      private var var_754:Boolean;
      
      private var _currentRoomOwner:Boolean;
      
      public function NavigatorData(param1:HabboNavigator)
      {
         var_1092 = new Array();
         var_1091 = new Array();
         var_913 = new Dictionary();
         super();
         _navigator = param1;
      }
      
      public function get createdFlatId() : int
      {
         return var_2193;
      }
      
      public function get eventMod() : Boolean
      {
         return var_2195;
      }
      
      public function startLoading() : void
      {
         this.var_754 = true;
      }
      
      public function isRoomFavourite(param1:int) : Boolean
      {
         return false;
      }
      
      public function set eventMod(param1:Boolean) : void
      {
         var_2195 = param1;
      }
      
      public function set roomEventData(param1:RoomEventData) : void
      {
         if(var_446 != null)
         {
            var_446.dispose();
         }
         var_446 = param1;
      }
      
      public function get popularTagsArrived() : Boolean
      {
         return var_134 != null && var_134 as PopularRoomTagsData != null;
      }
      
      public function get guestRoomSearchArrived() : Boolean
      {
         return var_134 != null && var_134 as GuestRoomSearchResultData != null;
      }
      
      public function get guestRoomSearchResults() : GuestRoomSearchResultData
      {
         return var_134 as GuestRoomSearchResultData;
      }
      
      public function get enteredGuestRoom() : GuestRoomData
      {
         return var_204;
      }
      
      public function get allCategories() : Array
      {
         return var_1092;
      }
      
      public function onRoomExit() : void
      {
         if(var_446 != null)
         {
            var_446.dispose();
            var_446 = null;
         }
         if(var_626 != null)
         {
            var_626.dispose();
            var_626 = null;
         }
         if(var_204 != null)
         {
            var_204.dispose();
            var_204 = null;
         }
         _currentRoomOwner = false;
      }
      
      public function set officialRooms(param1:OfficialRoomsData) : void
      {
         disposeCurrentMsg();
         var_134 = param1;
         var_754 = false;
      }
      
      public function get settings() : NavigatorSettingsMessageParser
      {
         return var_1577;
      }
      
      public function set popularTags(param1:PopularRoomTagsData) : void
      {
         disposeCurrentMsg();
         var_134 = param1;
         var_754 = false;
      }
      
      public function onRoomEnter(param1:RoomEntryInfoMessageParser) : void
      {
         var_626 = null;
         var_204 = null;
         _currentRoomOwner = false;
         if(param1.guestRoom)
         {
            _currentRoomOwner = param1.owner;
         }
         else
         {
            var_626 = param1.publicSpace;
            var_446 = null;
         }
      }
      
      public function get currentRoomOwner() : Boolean
      {
         return _currentRoomOwner;
      }
      
      public function onFavourites(param1:FavouritesMessageParser) : void
      {
         var _loc2_:int = 0;
         this.var_2339 = param1.limit;
         this.var_1347 = param1.favouriteRoomIds.length;
         this.var_913 = new Dictionary();
         for each(_loc2_ in param1.favouriteRoomIds)
         {
            this.var_913[_loc2_] = "yes";
         }
      }
      
      public function get popularTags() : PopularRoomTagsData
      {
         return var_134 as PopularRoomTagsData;
      }
      
      public function get enteredPublicSpace() : PublicRoomShortData
      {
         return var_626;
      }
      
      public function get hotRoomPopupOpen() : Boolean
      {
         return var_2194;
      }
      
      public function set guestRoomSearchResults(param1:GuestRoomSearchResultData) : void
      {
         disposeCurrentMsg();
         var_134 = param1;
         var_754 = false;
      }
      
      public function set avatarId(param1:int) : void
      {
         var_2035 = param1;
      }
      
      public function get canEditRoomSettings() : Boolean
      {
         return var_204 != null && _currentRoomOwner;
      }
      
      public function isLoading() : Boolean
      {
         return this.var_754;
      }
      
      public function get visibleCategories() : Array
      {
         return var_1091;
      }
      
      public function set categories(param1:Array) : void
      {
         var _loc2_:* = null;
         var_1092 = param1;
         var_1091 = new Array();
         for each(_loc2_ in var_1092)
         {
            if(_loc2_.visible)
            {
               var_1091.push(_loc2_);
            }
         }
      }
      
      public function get currentRoomRating() : int
      {
         return var_2190;
      }
      
      public function get publicSpaceNodeId() : int
      {
         return var_2192;
      }
      
      public function set settings(param1:NavigatorSettingsMessageParser) : void
      {
         var_1577 = param1;
      }
      
      private function disposeCurrentMsg() : void
      {
         if(var_134 == null)
         {
            return;
         }
         var_134.dispose();
         var_134 = null;
      }
      
      public function get roomEventData() : RoomEventData
      {
         return var_446;
      }
      
      public function favouriteChanged(param1:int, param2:Boolean) : void
      {
         var_913[param1] = !!param2 ? "yes" : null;
         var_1347 += !!param2 ? 1 : -1;
      }
      
      public function get officialRooms() : OfficialRoomsData
      {
         return var_134 as OfficialRoomsData;
      }
      
      public function get avatarId() : int
      {
         return var_2035;
      }
      
      public function isCurrentRoomFavourite() : Boolean
      {
         var _loc1_:int = 0;
         return false;
      }
      
      public function get officialRoomsArrived() : Boolean
      {
         return var_134 != null && var_134 as OfficialRoomsData != null;
      }
      
      public function set hotRoomPopupOpen(param1:Boolean) : void
      {
         var_2194 = param1;
      }
      
      public function set currentRoomRating(param1:int) : void
      {
         var_2190 = param1;
      }
      
      public function set hcMember(param1:Boolean) : void
      {
         var_2191 = param1;
      }
      
      public function set enteredRoom(param1:GuestRoomData) : void
      {
         if(var_204 != null)
         {
            var_204.dispose();
         }
         var_204 = param1;
      }
      
      public function isCurrentRoomHome() : Boolean
      {
         if(var_204 == null)
         {
            return false;
         }
         var _loc1_:int = 0;
         return this.var_1577.homeRoomId == _loc1_;
      }
      
      public function isFavouritesFull() : Boolean
      {
         return var_1347 >= var_2339;
      }
      
      public function set publicSpaceNodeId(param1:int) : void
      {
         var_2192 = param1;
      }
      
      public function get hcMember() : Boolean
      {
         return var_2191;
      }
      
      public function get canAddFavourite() : Boolean
      {
         return var_204 != null && !_currentRoomOwner;
      }
      
      public function set createdFlatId(param1:int) : void
      {
         var_2193 = param1;
      }
   }
}
