package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_418 >= var_302.length)
         {
            return "";
         }
         var petColor:IPetColor = var_302[var_418] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_593[var_1483] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_418 = param1.index;
         if(var_418 < 0 || var_418 > var_302.length)
         {
            var_418 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_609.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_132,onTextWindowEvent);
         var_609 = new Map();
         ...
         var_848 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_302 = [];
         var_593 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_848);
               if(_loc11_)
               {
                  var_593 = _loc11_.breeds;
                  var_302 = _loc11_.colors;
                  if(var_302 == null)
                  {
                     var_302 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_593.length)
         {
            _loc12_ = var_593[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_848,_loc12_.id),getRaceLocalizationKey(var_848,_loc12_.id)));
            _loc6_++;
         }
         var_609.add(var_848,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1002,onApproveNameResult);
         _loc5_ = var_609.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_302)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_609.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_418 >= 0 && var_418 < var_302.length)
         {
            _loc7_ = var_302[var_418] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_593[var_1483] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_848,_loc3_.id,_loc2_,AvatarScaleType.const_55);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_321,AvatarAction.const_735);
