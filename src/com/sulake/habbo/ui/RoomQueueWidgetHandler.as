package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_362:
               ...
               {
                  case RoomSessionQueueEvent.const_969:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_957:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_934) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_934) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1269) + 1;
                  }
               }
               else
               {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_591);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_682);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_659);
         return _loc1_;
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_591:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_957);
               break;
            case RoomWidgetRoomQueueMessage.const_682:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_969);
               break;
            case RoomWidgetRoomQueueMessage.const_659:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_202,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_362];
      }
