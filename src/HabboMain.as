package
{
   ...
   public class HabboMain extends Sprite
   {
      ...
      private function prepareCore() : void
      {
         ...
         _loc1_.addEventListener(AssetLoaderEvent.const_36,configLoadedHandler);
      ...
      private function configLoadedHandler(param1:AssetLoaderEvent) : void
      {
         ...
         _loc2_.removeEventListener(AssetLoaderEvent.const_36,configLoadedHandler);
         _loc2_.removeEventListener(AssetLoaderEvent.const_47,configLoadedHandler);
         if(param1.type == AssetLoaderEvent.const_36)
         {
            _loc3_ = var_27.assets.getAssetByName(_loc2_.assetName) as XmlAsset;
            _loc4_ = _loc3_.content as XML;
            var_27.readConfigDocument(_loc4_,var_1);
         }
         else
         {
            var_27.error("Failed to download external configuration document " + _loc2_.assetLoader.url + "!",true,Core.const_863);
            var_27.events.dispatchEvent(new Event(Component.COMPONENT_EVENT_ERROR));
         }
      }
      ...
      private function initializeCore() : void
      {
         ...
         catch(error:Error)
         {
            ...
            Core.crash("Failed to initialize the core: " + error.message,Core.const_980,error);
         }
      }
      ...
      protected function onAddedToStage(param1:Event = null) : void
      {
         ...
         {
            trackLoginStep(Habbo.const_28);
            Habbo.reportCrash("Failed to prepare the core: " + error.message,Core.const_980,error);
            Core.dispose();
         }
      }
