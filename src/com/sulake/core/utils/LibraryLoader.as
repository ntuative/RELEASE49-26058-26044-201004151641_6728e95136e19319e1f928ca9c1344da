package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1402:int = 5;
      ...
      {
         var_2288 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_377,bytesTotal,bytesLoaded));
      }
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_480 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(_debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_377,bytesTotal,bytesLoaded));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               debug("Load event STATUS " + var_377 + " for file \"" + var_14.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_14.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_377,bytesTotal,bytesLoaded));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_14.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_377,bytesTotal,bytesLoaded));
               break;
            case IOErrorEvent.IO_ERROR:
               failure("IO Error, send or load operation failed for file \"" + var_14.contentLoaderInfo.url + "\"");
               if(!handleHttpStatus(var_377))
               {
         ...
         if(var_454 == LibraryLoader.STATE_READY)
         {
            ...
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_377,bytesTotal,bytesLoaded));
         }
      }
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_951 > 0)
            {
               var_14.close();
               var_14.unload();
               addRequestCounterToUrlRequest(var_323,const_1402 - var_951);
               var_14.load(var_323,var_15);
               --var_951;
               return true;
            }
