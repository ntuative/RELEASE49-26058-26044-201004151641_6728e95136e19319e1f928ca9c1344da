package com.sulake.core.window.utils
{
   import com.sulake.core.window.enum.WindowParam;
   import flash.utils.Dictionary;
   
   public class ParamCodeTable extends WindowParam
   {
       
      
      public function ParamCodeTable()
      {
         super();
      }
      
      public static function fillTables(param1:Dictionary, param2:Dictionary = null) : void
      {
         var _loc3_:* = null;
         param1["null"] = const_112;
         param1["bound_to_parent_rect"] = const_91;
         param1["child_window"] = const_824;
         param1["embedded_controller"] = const_319;
         param1["resize_to_accommodate_children"] = const_64;
         param1["input_event_processor"] = const_50;
         param1["internal_event_handling"] = const_558;
         param1["mouse_dragging_target"] = const_201;
         param1["mouse_dragging_trigger"] = const_341;
         param1["mouse_scaling_target"] = const_248;
         param1["mouse_scaling_trigger"] = const_404;
         param1["horizontal_mouse_scaling_trigger"] = const_199;
         param1["vertical_mouse_scaling_trigger"] = const_221;
         param1["observe_parent_input_events"] = const_830;
         param1["optimize_region_to_layout_size"] = const_407;
         param1["parent_window"] = const_959;
         param1["relative_horizontal_scale_center"] = const_168;
         param1["relative_horizontal_scale_fixed"] = const_125;
         param1["relative_horizontal_scale_move"] = const_286;
         param1["relative_horizontal_scale_strech"] = const_274;
         param1["relative_scale_center"] = const_1005;
         param1["relative_scale_fixed"] = const_653;
         param1["relative_scale_move"] = const_976;
         param1["relative_scale_strech"] = const_960;
         param1["relative_vertical_scale_center"] = const_170;
         param1["relative_vertical_scale_fixed"] = const_116;
         param1["relative_vertical_scale_move"] = const_291;
         param1["relative_vertical_scale_strech"] = const_263;
         param1["on_resize_align_left"] = const_512;
         param1["on_resize_align_right"] = const_442;
         param1["on_resize_align_center"] = const_459;
         param1["on_resize_align_top"] = const_681;
         param1["on_resize_align_bottom"] = const_455;
         param1["on_resize_align_middle"] = const_382;
         param1["on_accommodate_align_left"] = const_968;
         param1["on_accommodate_align_right"] = const_400;
         param1["on_accommodate_align_center"] = const_557;
         param1["on_accommodate_align_top"] = const_1009;
         param1["on_accommodate_align_bottom"] = WINDOW_PARAM_ON_ACCOMMODATE_ALIGN_BOTTOM;
         param1["on_accommodate_align_middle"] = const_517;
         param1["route_input_events_to_parent"] = const_373;
         param1["use_parent_graphic_context"] = const_34;
         param1["draggable_with_mouse"] = const_946;
         param1["scalable_with_mouse"] = const_1004;
         param1["reflect_horizontal_resize_to_parent"] = const_427;
         param1["reflect_vertical_resize_to_parent"] = WINDOW_PARAM_REFLECT_VERTICAL_RESIZE_TO_PARENT;
         param1["reflect_resize_to_parent"] = const_240;
         if(param2 != null)
         {
            for(_loc3_ in param1)
            {
               if(param2[param1[_loc3_]] != null)
               {
                  if(param1[_loc3_] != 0)
                  {
                     Logger.log("Conflictiong flags in window params: \"" + _loc3_ + "\"!");
                  }
               }
               param2[param1[_loc3_]] = _loc3_;
            }
         }
      }
   }
}
